(function(ctx) {
	...
	var challenge = {
		config: {
			id: 11,
			title: 'The Maze',
			resume: 'You are lost in a living maze.<br>Your robot must found the output door',
			max_participant: 1,
		...
		var get = function(h, w) {
			...
			function canGo(dir, x, y) {
				try {
					var _p;
					if (dir == LEFT) {
						if (l[x - 2][y] == 'x' && l[x - 1][y + 1] == 'x' && l[x - 1][y - 1] == 'x') _p = { x: x - 1, y: y };
					}
					if (dir == RIGHT) {
						if (l[x + 2][y] == 'x' && l[x + 1][y + 1] == 'x' && l[x + 1][y - 1] == 'x') _p = { x: x + 1, y: y };
					}
					if (dir == DOWN) {
						if (l[x][y - 2] == 'x' && l[x - 1][y - 1] == 'x' && l[x + 1][y - 1] == 'x') _p = { x: x, y: y - 1 };
					}
					if (dir == UP) {
						if (l[x][y + 2] == 'x' && l[x - 1][y + 1] == 'x' && l[x + 1][y + 1] == 'x') _p = { x: x, y: y + 1 };
					}
